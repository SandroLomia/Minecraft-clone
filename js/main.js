...
function createBlockTexture(blockType, color) {
    ...
    if (blockType === BLOCK_TYPES.GRASS) {
        ...
        const oreColor = blockType === BLOCK_TYPES.COAL_ORE ? '#222' :
                         ...
                         (blockType === BLOCK_TYPES.GOLD_ORE ? '#fce166' : '#5ff2f2'));
        for (let i = 0; i < 6; i++) {
...
for (let i = 0; i < 20; i++) {
    ...
    for (let j = 0; j < size; j++) {
        ...
        part.scale.set(Math.random() * 4 + 2, Math.random() * 2 + 1, Math.random() * 4 + 2);
        cloud.add(part);
    }
    clouds.add(cloud);
    cloudList.push({ mesh: cloud, x: nx, z: nz });
}
scene.add(clouds);
        ...
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            for (let y = 0; y < CHUNK_HEIGHT; y++) {
                if (y < height - 4) {
                    ...
                    else if (ironNoise > 0.7 && y < height - 6) block = BLOCK_TYPES.IRON_ORE;
...
function updatePhysics() {
    ...
    // Crouching height interpolation
    ...
    if (playerVelocity.x !== 0) {
        ...
        if (isBlockSolid(getBlockAt(checkX, playerY, camera.position.z)) ||
            isBlockSolid(getBlockAt(checkX, playerY + (currentPlayerHeight > 1 ? 1 : 0.5), camera.position.z))) {
            playerVelocity.x = 0;
    ...
    if (playerVelocity.z !== 0) {
        ...
        if (isBlockSolid(getBlockAt(camera.position.x, playerY, checkZ)) ||
            isBlockSolid(getBlockAt(camera.position.x, playerY + (currentPlayerHeight > 1 ? 1 : 0.5), checkZ))) {
            playerVelocity.z = 0;
