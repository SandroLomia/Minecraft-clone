...
function createBlockTexture(blockType, color) {
    ...
    if (blockType === BLOCK_TYPES.GRASS) {
        ...
        ctx.fillRect(0, 0, 16, 16);
        const oreColor = blockType === BLOCK_TYPES.COAL_ORE ? '#222' : (blockType === BLOCK_TYPES.IRON_ORE ? '#d4a373' : (blockType === BLOCK_TYPES.GOLD_ORE ? '#ffd700' : '#00ffff'));
        for (let i = 0; i < 6; i++) {
        ...
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            for (let y = 0; y < CHUNK_HEIGHT; y++) {
                if (y < height - 4) {
                    ...
                    if (y < height - 10 && diamondNoise > 0.85) {
                        chunk.setBlock(x, y, z, BLOCK_TYPES.DIAMOND_ORE);
                    } else if (y < height - 7 && goldNoise > 0.82) {
                        chunk.setBlock(x, y, z, BLOCK_TYPES.GOLD_ORE);
                    } else if (y < height - 4 && ironNoise > 0.75) {
...
function createChunkMesh(chunk) {
    ...
    for (const [blockTypeStr, matrices] of Object.entries(instancedMeshes)) {
        ...
        const isFlower = blockType === BLOCK_TYPES.FLOWER_RED || blockType === BLOCK_TYPES.FLOWER_YELLOW;
        const material = new THREE.MeshLambertMaterial({
            map: BLOCK_TEXTURES[blockType],
            transparent: blockType === BLOCK_TYPES.GLASS || blockType === BLOCK_TYPES.WATER || blockType === BLOCK_TYPES.LEAVES || isFlower,
            opacity: blockType === BLOCK_TYPES.GLASS ? 0.6 : (blockType === BLOCK_TYPES.WATER ? 0.6 : 1),
            alphaTest: (blockType === BLOCK_TYPES.LEAVES || isFlower) ? 0.5 : 0,
            side: (blockType === BLOCK_TYPES.LEAVES || isFlower) ? THREE.DoubleSide : THREE.FrontSide
        });
...
function performAction(action) {
    ...
    if (intersects.length > 0) {
        ...
        if (action === 'break') {
            ...
            const playerFootY = Math.floor(playerPos.y - currentPlayerHeight + 0.1);
            if (Math.floor(playerPos.x) === x && Math.floor(playerPos.z) === z &&
               (y === playerHeadY || y === playerFootY)) {
                return;
            }
...
function updatePhysics() {
    ...
    const playerFootY = camera.position.y - currentPlayerHeight + 0.1;
