...
function createBlockTexture(blockType, color) {
    ...
    if (isFoliage(blockType)) {
        ...
        if (blockType === BLOCK_TYPES.RED_FLOWER) {
            ...
            const grassColors = ['#43a047', '#2e7d32', '#66bb6a'];
            for (let x = 2; x < 14; x += 2) {
                const height = 8 + Math.floor(Math.sin(x) * 4);
                ctx.fillStyle = grassColors[x % grassColors.length];
        ...
        if (blockType === BLOCK_TYPES.GRASS) {
            ...
            ctx.fillRect(0, 0, 16, 16);
            for (let i = 0; i < 6; i++) {
                const rx = Math.floor(Math.sin(i * 3.7 + 1) * 5 + 7);
                const ry = Math.floor(Math.cos(i * 2.3 + 2) * 5 + 7);
                ctx.fillStyle = '#222';
            ...
            ctx.fillRect(0, 0, 16, 16);
            for (let i = 0; i < 6; i++) {
                const rx = Math.floor(Math.sin(i * 4.1 + 2) * 5 + 7);
                const ry = Math.floor(Math.cos(i * 1.9 + 1) * 5 + 7);
                ctx.fillStyle = '#d4af37';
                ctx.fillRect(rx, ry, 3, 2);
            }
        }

        if (blockType !== BLOCK_TYPES.DIRT && blockType !== BLOCK_TYPES.GRASS && blockType !== BLOCK_TYPES.SAND && blockType !== BLOCK_TYPES.LEAVES && blockType !== BLOCK_TYPES.GLASS && blockType !== BLOCK_TYPES.BRICK) {
            for (let i = 0; i < 60; i++) {
                const x = (i * 7) % 16;
                const y = (i * 13) % 16;
        ...
        if (blockType !== BLOCK_TYPES.GLASS) {
...
// Foliage billboard geometry
function createFoliageGeometry() {
    const p1 = new THREE.PlaneGeometry(0.8, 0.8).toNonIndexed();
    p1.rotateY(Math.PI / 4);
    p1.translate(0.5, 0.4, 0.5);

    const p2 = new THREE.PlaneGeometry(0.8, 0.8).toNonIndexed();
    p2.rotateY(-Math.PI / 4);
    p2.translate(0.5, 0.4, 0.5);
...
function createClouds() {
    const cloudMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.7, fog: true });
    const cloudBoxGeo = new THREE.BoxGeometry(12, 3, 12);
    for (let i = 0; i < 15; i++) {
        const cloud = new THREE.Mesh(cloudBoxGeo, cloudMat);
        cloud.position.set(
            (Math.random() - 0.5) * 160,
            22 + Math.random() * 3,
            (Math.random() - 0.5) * 160
...
// First-person held block model
const heldBlockGroup = new THREE.Group();
const heldBlockGeo = new THREE.BoxGeometry(0.35, 0.35, 0.35);
let heldBlockMaterial = new THREE.MeshLambertMaterial({ map: BLOCK_TEXTURES[BLOCK_TYPES.STONE] });
const heldBlockMesh = new THREE.Mesh(heldBlockGeo, heldBlockMaterial);
...
function updateHeldBlock() {
    ...
    heldBlockMesh.material.transparent = isBlockTransparent(selectedBlock);
    heldBlockMesh.material.alphaTest = isFoliage(selectedBlock) || selectedBlock === BLOCK_TYPES.LEAVES ? 0.5 : 0;
    heldBlockMesh.material.opacity = selectedBlock === BLOCK_TYPES.GLASS || selectedBlock === BLOCK_TYPES.WATER ? 0.6 : 1.0;
...
function createOscillatorSound(frequency, duration, type = 'sine', gain = 0.07, attack = 0.005) {
    try {
        ...
        // Silently catch audio restrictions before user interaction
    }
}
...
function playSound(action) {
    if (action === 'break') {
        ...
        createOscillatorSound(110 + Math.random() * 30, 0.04, 'triangle', 0.02, 0.001);
    } else if (action === 'splash') {
        createOscillatorSound(180, 0.15, 'sine', 0.06, 0.01);
        createOscillatorSound(90, 0.2, 'triangle', 0.04, 0.01);
    } else if (action === 'click') {
        createOscillatorSound(520, 0.03, 'sine', 0.03, 0.001);
    }
}
...
function updateEnvironment() {
    ...
    const currentSubmerged = getBlockAt(camera.position.x, camera.position.y, camera.position.z) === BLOCK_TYPES.WATER;

    if (currentSubmerged) {
        const waterColor = new THREE.Color(0x0a355c);
    ...
    if (cloudsGroup.position.x > 80) cloudsGroup.position.x = -80;
}
...
function generateTerrain(chunkX, chunkZ) {
    ...
    for (let x = 0; x < CHUNK_SIZE; x++) {
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            // Flowers & Tall Grass on surface
            if (!isBeach && height > 6) {
                const plantNoise = textureNoise.noise2D(worldX * 0.8 + 10, worldZ * 0.8 + 10);
                if (plantNoise > 0.6) {
                    chunk.setBlock(x, height, z, BLOCK_TYPES.TALL_GRASS);
                } else if (plantNoise > 0.48) {
                    chunk.setBlock(x, height, z, BLOCK_TYPES.RED_FLOWER);
                } else if (plantNoise > 0.4) {
...
function createChunkMesh(chunk) {
    ...
    for (const [blockTypeStr, matrices] of Object.entries(instancedMeshes)) {
        ...
        const isFoliageBlock = isFoliage(blockType);
        const geometry = isFoliageBlock ? foliageGeometry : cubeGeometry;

        const material = new THREE.MeshLambertMaterial({
            map: BLOCK_TEXTURES[blockType],
            transparent: isBlockTransparent(blockType),
            opacity: blockType === BLOCK_TYPES.GLASS || blockType === BLOCK_TYPES.WATER ? 0.6 : 1.0,
            alphaTest: isFoliageBlock || blockType === BLOCK_TYPES.LEAVES ? 0.5 : 0,
            side: isFoliageBlock || blockType === BLOCK_TYPES.LEAVES ? THREE.DoubleSide : THREE.FrontSide
        });
...
inventoryBlocks.forEach((type, index) => {
    ...
    slotNum.textContent = (index + 1).toString();
...
// Mouse wheel inventory switching
window.addEventListener('wheel', (e) => {
    if (!controls.isLocked) return;
    if (e.deltaY > 0) {
        selectInventorySlot((selectedBlockSlot + 1) % inventoryBlocks.length);
    } else if (e.deltaY < 0) {
        selectInventorySlot((selectedBlockSlot - 1 + inventoryBlocks.length) % inventoryBlocks.length);
    }
});
...
function handleMovement() {
    ...
    if (isMoving && isGrounded) {
        ...
        const bobX = Math.sin(bobTimer) * 0.04;
        const bobY = Math.abs(Math.cos(bobTimer)) * 0.05;
        bobOffset.set(bobX, bobY, 0);

        footstepTimer += isSprinting ? 0.22 : 0.14;
...
function updatePhysics() {
    // Remove previous frame bob offset before running physics
    camera.position.sub(bobOffset);

    const inWater = getBlockAt(camera.position.x, camera.position.y - 0.5, camera.position.z) === BLOCK_TYPES.WATER;
    playerVelocity.y += inWater ? GRAVITY * 0.25 : GRAVITY;

    const nextPos = camera.position.clone().add(playerVelocity);

    // Vertical collisions against solid blocks
    const footY = nextPos.y - currentPlayerHeight;
    const checkFootBlock = getBlockAt(nextPos.x, footY, nextPos.z);

    if (isBlockSolid(checkFootBlock)) {
        playerVelocity.y = 0;
        isGrounded = true;
        nextPos.y = Math.ceil(footY) + currentPlayerHeight;
    } else {
        isGrounded = false;
    }
...
function animateHeldBlock() {
    if (isSwinging) {
        ...
        heldBlockGroup.rotation.x = -swingAngle;
        heldBlockGroup.position.z = -swingAngle * 0.2;
        if (swingProgress >= 1) {
            isSwinging = false;
            swingProgress = 0;
            heldBlockGroup.rotation.x = 0;
            heldBlockGroup.position.z = 0;
        }
    } else {
        heldBlockMesh.position.y = -0.35 + bobOffset.y * 0.5;
        heldBlockMesh.position.x = 0.4 + bobOffset.x * 0.5;
    }
}
...
console.log('Minecraft Web Game initialized');
