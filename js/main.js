        ...
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            for (let y = 0; y < CHUNK_HEIGHT; y++) {
                if (y < height - 4) {
                    ...
                    if (y < height - 10 && diamondNoise > 0.75) {
                        chunk.setBlock(x, y, z, BLOCK_TYPES.DIAMOND_ORE);
                    } else if (y < height - 8 && goldNoise > 0.72) {
                        chunk.setBlock(x, y, z, BLOCK_TYPES.GOLD_ORE);
                    } else if (y < height - 6 && ironNoise > 0.68) {
...
function isBlockTransparent(type) {
    return type === BLOCK_TYPES.AIR || type === BLOCK_TYPES.GLASS || type === BLOCK_TYPES.LEAVES ||
           type === BLOCK_TYPES.WATER || type === BLOCK_TYPES.FLOWER_RED || type === BLOCK_TYPES.FLOWER_YELLOW;
}

function isBlockSolid(type) {
    return type !== BLOCK_TYPES.AIR && type !== BLOCK_TYPES.WATER &&
           type !== BLOCK_TYPES.FLOWER_RED && type !== BLOCK_TYPES.FLOWER_YELLOW;
}
    ...
    for (const [blockTypeStr, matrices] of Object.entries(instancedMeshes)) {
        ...
        const material = new THREE.MeshLambertMaterial({
            ...
            transparent: blockType === BLOCK_TYPES.GLASS || blockType === BLOCK_TYPES.WATER ||
                         blockType === BLOCK_TYPES.LEAVES || blockType === BLOCK_TYPES.FLOWER_RED ||
                         blockType === BLOCK_TYPES.FLOWER_YELLOW,
            opacity: blockType === BLOCK_TYPES.GLASS ? 0.6 : (blockType === BLOCK_TYPES.WATER ? 0.6 : 1),
            alphaTest: (blockType === BLOCK_TYPES.LEAVES || blockType === BLOCK_TYPES.FLOWER_RED ||
                        blockType === BLOCK_TYPES.FLOWER_YELLOW) ? 0.5 : 0,
            side: (blockType === BLOCK_TYPES.LEAVES || blockType === BLOCK_TYPES.FLOWER_RED ||
                   blockType === BLOCK_TYPES.FLOWER_YELLOW) ? THREE.DoubleSide : THREE.FrontSide
        });
...
function performAction(action) {
    ...
    if (intersects.length > 0) {
        ...
        if (action === 'break') {
            ...
            const playerBottom = playerPos.y - currentPlayerHeight;
            const playerTop = playerPos.y;

            if (Math.floor(playerPos.x) === x && Math.floor(playerPos.z) === z &&
                y >= Math.floor(playerBottom) && y < Math.ceil(playerTop)) {
                return;
            }
