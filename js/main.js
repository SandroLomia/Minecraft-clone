...
function createBlockTexture(blockType, color) {
    ...
    if (blockType === BLOCK_TYPES.GRASS) {
        ...
        const oreColor = blockType === BLOCK_TYPES.COAL_ORE ? '#222' :
                         ...
                         blockType === BLOCK_TYPES.GOLD_ORE ? '#fce043' : '#00ffff';
        for (let i = 0; i < 8; i++) {
    ...
    for (let x = 0; x < CHUNK_SIZE; x++) {
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            if (!isBeach && height > 6 && x > 1 && x < CHUNK_SIZE - 2 && z > 1 && z < CHUNK_SIZE - 2) {
                const envNoise = textureNoise.noise2D(worldX * 0.5, worldZ * 0.5);
                if (envNoise > 0.8) { // Trees
                    // Trunk
                ...
                } else if (envNoise > 0.6) { // Flowers
                    const flowerType = envNoise > 0.7 ? BLOCK_TYPES.FLOWER_RED : BLOCK_TYPES.FLOWER_YELLOW;
...
function animate() {
    ...
    if (isGrounded && (Math.abs(playerVelocity.x) > 0.01 || Math.abs(playerVelocity.z) > 0.01)) {
        const bobSpeed = isSprinting ? 0.15 : (isCrouching ? 0.05 : 0.1);
        bobTimer += bobSpeed;
        const bobAmount = isSprinting ? 0.08 : (isCrouching ? 0.02 : 0.05);
        camera.position.y += Math.sin(bobTimer) * bobAmount * 0.5;
        camera.position.x += Math.cos(bobTimer * 0.5) * bobAmount * 0.2;
    } else {
        bobTimer = 0;
    }
    updateVisibleChunks();
