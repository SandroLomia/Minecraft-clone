...
function createBlockTexture(blockType, color) {
    ...
    if (blockType === BLOCK_TYPES.GRASS) {
        ...
        const oreColor = blockType === BLOCK_TYPES.COAL_ORE ? '#222' :
                         (blockType === BLOCK_TYPES.IRON_ORE ? '#d8d8d8' :
...
for (let i = 0; i < 15; i++) {
    createCloud(Math.random() * 200 - 100, Math.random() * 200 - 100);
}
        ...
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            for (let y = 0; y < CHUNK_HEIGHT; y++) {
                if (y < height - 4) {
                    ...
                    if (y < height - 10 && oreNoise > 0.85) {
                        chunk.setBlock(x, y, z, BLOCK_TYPES.DIAMOND_ORE);
                    } else if (y < height - 7 && oreNoise > 0.8) {
                        chunk.setBlock(x, y, z, BLOCK_TYPES.GOLD_ORE);
                    } else if (y < height - 4 && oreNoise > 0.75) {
            ...
            if (!isBeach && height > 6 && x > 1 && x < CHUNK_SIZE - 2 && z > 1 && z < CHUNK_SIZE - 2) {
                ...
                if (treeNoise > 0.8) {
                    ...
                    const flowerNoise = textureNoise.noise2D(worldX * 2, worldZ * 2);
                    if (flowerNoise > 0.5) {
                        chunk.setBlock(x, height, z, flowerNoise > 0.75 ? BLOCK_TYPES.FLOWER_RED : BLOCK_TYPES.FLOWER_YELLOW);
                    }
                }
            }
        }
...
function animate() {
    ...
    // View Bobbing setup
    camera.position.y -= lastBobOffset;

    handleMovement();
    updatePhysics();

    // Apply View Bobbing
    const isMoving = Math.abs(playerVelocity.x) > 0 || Math.abs(playerVelocity.z) > 0;
