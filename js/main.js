...
function createBlockTexture(blockType, color) {
    ...
    if (blockType === BLOCK_TYPES.GRASS) {
        ...
        ctx.fillRect(0, 0, 16, 16);
        for (let i = 0; i < 7; i++) {
            const rx = Math.floor(Math.random() * 11) + 2;
            const ry = Math.floor(Math.random() * 11) + 2;
            ctx.fillStyle = blockType === BLOCK_TYPES.COAL_ORE ? '#222' : adjustHexColor(color, 15);
    ...
    if (blockType !== BLOCK_TYPES.DIRT && blockType !== BLOCK_TYPES.GRASS && blockType !== BLOCK_TYPES.SAND && blockType !== BLOCK_TYPES.LEAVES && blockType !== BLOCK_TYPES.WOOD && blockType !== BLOCK_TYPES.FLOWER_RED && blockType !== BLOCK_TYPES.FLOWER_YELLOW) {
        for (let i = 0; i < 80; i++) {
...
Object.entries(BLOCK_TEXTURES).forEach(([typeStr, texture]) => {
    const type = parseInt(typeStr);
    const isTransparent = type === BLOCK_TYPES.GLASS || type === BLOCK_TYPES.WATER || type === BLOCK_TYPES.LEAVES || type === BLOCK_TYPES.FLOWER_RED || type === BLOCK_TYPES.FLOWER_YELLOW;
...
const cloudGeometry = new THREE.BoxGeometry(10, 2, 15);
...
for (let i = 0; i < 20; i++) {
    ...
    cloud.scale.set(Math.random() * 2 + 1, 1, Math.random() * 2 + 1);
    clouds.add(cloud);
}
scene.add(clouds);
    ...
    for (let x = 0; x < CHUNK_SIZE; x++) {
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            for (let y = 0; y < CHUNK_HEIGHT; y++) {
                if (y < height - 4) {
                    const noiseCoord = worldX * 0.2 + (y + worldZ) * 0.2;
                    const oreNoise = simplex.noise2D(worldX * 0.2, (y + worldZ) * 0.2);
                    let type = BLOCK_TYPES.STONE;

                    if (oreNoise > 0.8 && y < height - 10) type = BLOCK_TYPES.DIAMOND_ORE;
                    else if (oreNoise > 0.75 && y < height - 8) type = BLOCK_TYPES.GOLD_ORE;
                    else if (oreNoise > 0.7 && y < height - 6) type = BLOCK_TYPES.IRON_ORE;
                    else if (oreNoise > 0.6) type = BLOCK_TYPES.COAL_ORE;
            ...
            if (!isBeach && height > 6) {
                ...
                if (treeNoise > 0.8 && x > 1 && x < CHUNK_SIZE - 2 && z > 1 && z < CHUNK_SIZE - 2) {
                    ...
                    for (let ty = 1; ty <= 5; ty++) {
                        if (chunk.getBlock(x, height + ty, z) !== BLOCK_TYPES.AIR) {
                            canPlaceTree = false;
                            break;
                        }
                    }
                    ...
                    const flowerType = textureNoise.noise2D(worldX * 0.1, worldZ * 0.1) > 0 ? BLOCK_TYPES.FLOWER_RED : BLOCK_TYPES.FLOWER_YELLOW;
...
function updatePhysics() {
    ...
    const playerBottom = nextPos.y - currentPlayerHeight + 0.1;
    const playerTop = nextPos.y - 0.1;

    // Check X direction
    if (playerVelocity.x !== 0) {
        const checkX = nextPos.x + (playerVelocity.x > 0 ? checkRadius : -checkRadius);
        if (isBlockSolid(getBlockAt(checkX, playerBottom, camera.position.z)) ||
            isBlockSolid(getBlockAt(checkX, playerTop, camera.position.z))) {
            playerVelocity.x = 0;
    ...
    if (playerVelocity.z !== 0) {
        ...
        if (isBlockSolid(getBlockAt(camera.position.x, playerBottom, checkZ)) ||
            isBlockSolid(getBlockAt(camera.position.x, playerTop, checkZ))) {
            playerVelocity.z = 0;
