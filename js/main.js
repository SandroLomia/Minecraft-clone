...
function createBlockTexture(blockType, color) {
    ...
    if (blockType === BLOCK_TYPES.GRASS) {
        ...
        const oreColor = blockType === BLOCK_TYPES.COAL_ORE ? '#222' :
                         (blockType === BLOCK_TYPES.IRON_ORE ? '#d0d0d0' :
        ...
        const petalColor = blockType === BLOCK_TYPES.FLOWER_RED ? '#ff3333' : '#ffff33';
        ctx.fillStyle = '#228B22'; // Stem
        ctx.fillRect(7, 8, 2, 8);
        ctx.fillStyle = petalColor;
        ctx.fillRect(6, 4, 4, 4);
        ctx.fillRect(4, 6, 8, 2);
        ctx.fillStyle = '#333'; // Center
...
function updateEnvironment() {
    ...
    if (BLOCK_MATERIALS[BLOCK_TYPES.WATER]) {
        BLOCK_MATERIALS[BLOCK_TYPES.WATER].opacity = 0.3 + Math.sin(Date.now() * 0.002) * 0.2;
    }
}
...
function generateTerrain(chunkX, chunkZ) {
    ...
    for (let x = 0; x < CHUNK_SIZE; x++) {
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            // Flowers
            if (!isBeach && height > 6 && chunk.getBlock(x, height, z) === BLOCK_TYPES.AIR) {
                const flowerNoise = textureNoise.noise2D(worldX * 0.9, worldZ * 0.9);
                if (flowerNoise > 0.85) {
...
function createChunkMesh(chunk) {
    ...
    for (const [blockTypeStr, matrices] of Object.entries(instancedMeshes)) {
        ...
        const mesh = new THREE.InstancedMesh(geometry, BLOCK_MATERIALS[blockType], matrices.length);
...
function handleMovement() {
    ...
    // Smooth height change for crouching
