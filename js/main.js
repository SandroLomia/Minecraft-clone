...
function createBlockTexture(blockType, color) {
    ...
    if (blockType === BLOCK_TYPES.GRASS) {
        ...
        const oreColor = blockType === BLOCK_TYPES.COAL_ORE ? '#222' :
                         blockType === BLOCK_TYPES.IRON_ORE ? '#d8b090' :
...
// Clouds
const clouds = [];
const cloudGeometry = new THREE.BoxGeometry(10, 2, 6);
        ...
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            for (let y = 0; y < CHUNK_HEIGHT; y++) {
                if (y < height - 4) {
                    ...
                    if (y < height - 10 && diamondNoise > 0.8) {
                        chunk.setBlock(x, y, z, BLOCK_TYPES.DIAMOND_ORE);
                    } else if (y < height - 7 && goldNoise > 0.75) {
                        chunk.setBlock(x, y, z, BLOCK_TYPES.GOLD_ORE);
                    } else if (y < height - 4 && ironNoise > 0.7) {
...
function createChunkMesh(chunk) {
    ...
    for (const [blockTypeStr, matrices] of Object.entries(instancedMeshes)) {
        ...
        const isTransparent = blockType === BLOCK_TYPES.GLASS || blockType === BLOCK_TYPES.WATER || blockType === BLOCK_TYPES.LEAVES || blockType === BLOCK_TYPES.FLOWER_RED || blockType === BLOCK_TYPES.FLOWER_YELLOW;
...
inventoryBlocks.forEach((type, index) => {
    ...
    // For flowers, we want a special UI representation since they are transparent
    if (type === BLOCK_TYPES.FLOWER_RED || type === BLOCK_TYPES.FLOWER_YELLOW) {
        slot.style.backgroundColor = '#4caf50';
        const inner = document.createElement('div');
        inner.style.width = '60%';
        inner.style.height = '60%';
        inner.style.margin = '20%';
        inner.style.backgroundColor = `#${BLOCK_COLORS[type].toString(16).padStart(6, '0')}`;
...
function handleMovement() {
    ...
    if (direction.length() > 0) {
        ...
        if (isGrounded) {
            ...
            bobOffset.x = Math.cos(bobTimer * 0.5) * bobAmount * 0.5;
        }
    } else {
        bobTimer = 0;
...
function updatePhysics() {
    ...
    // Reset camera position from bobbing before physics
