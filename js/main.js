...
selectionBox.raycast = () => null; // Prevent raycaster from hitting the selection box
    ...
    for (let lx = -2; lx <= 2; lx++) {
        for (let lz = -2; lz <= 2; lz++) {
            for (let ly = trunkHeight - 1; ly <= trunkHeight + 1; ly++) {
                // Circular-ish leaves
                if (Math.abs(lx) + Math.abs(lz) + Math.abs(ly - trunkHeight) > 3) continue;
        ...
        for (let z = 0; z < CHUNK_SIZE; z++) {
            ...
            for (let y = 0; y < CHUNK_HEIGHT; y++) {
                if (y < height - 4) {
                    ...
                    // Only set to AIR if the block is currently empty
                    // This prevents overwriting tree parts generated by other columns
            ...
            // Tree generation must happen AFTER the initial column is generated to avoid overwriting by the loop
            const worldBlockType = chunk.getBlock(x, height - 1, z);
            if (worldBlockType === BLOCK_TYPES.GRASS && !isBeach) {
                if (Math.random() < 0.015 && x > 1 && x < CHUNK_SIZE - 2 && z > 1 && z < CHUNK_SIZE - 2) {
...
function animate() {
    ...
    const lerpFactor = Math.max(0, Math.min(1, (sunHeight + 0.2) * 2));

    const dayColor = new THREE.Color(0x87CEEB);
    const nightColor = new THREE.Color(0x050510);
    const skyColor = dayColor.clone().lerp(nightColor, 1 - lerpFactor);

    scene.background = skyColor;
    scene.fog.color = skyColor;
    directionalLight.intensity = lerpFactor * 0.8;
    ambientLight.intensity = 0.2 + lerpFactor * 0.4;
